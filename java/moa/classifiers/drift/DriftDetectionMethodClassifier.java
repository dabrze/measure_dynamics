...
import moa.classifiers.core.driftdetection.PageHinkleyWindowDM;
...
public class DriftDetectionMethodClassifier extends AbstractClassifier implements MultiClassClassifier {
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        boolean prediction;
        double[] classVotes = this.classifier.getVotesForInstance(inst); 
        if (Utils.maxIndex(classVotes) == trueClass) {
            prediction = true;
        } else {
            prediction = false;
        }
        if(this.driftDetectionMethod instanceof PageHinkleyWindowDM){
        	((PageHinkleyWindowDM)this.driftDetectionMethod).input(new InstanceExample(inst), classVotes);
        } else {
